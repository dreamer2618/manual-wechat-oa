...
<font color="red">
####修改messagemanagement工程时需要注意以下几点，否则推送将不会成功：
####- 启动DDpush服务(DDpush不需要配置，启动即可)
####- home工程中(视为推送客户端)需要确认参数 proper_platform_rmi_port(Linux系统路径：tomcat/bin/catalina.sh | windows系统路径:tomcat/bin/catalina.bat)。此参数需要与messagemanagement工程(视为服务端)中的conf.properties中的参数platform.rmi.port一致(路径: messagemanagement/WEB-INF/classes)</font>
...
说明
--
mobile-platform由各个项目的开发者提供，相应的配置参数由现场部署文件提供者提供。
<font color="red">
####- home工程中(视为推送客户端)需要确认参数 proper_platform_rmi_host(Linux系统路径：tomcat/bin/catalina.sh | windows系统路径:tomcat/bin/catalina.bat)。此参数需要与mobile-platform工程(视为服务端)中的tomcat中的参数-Djava.rmi.server.hostname 以及 proper_platform_rmi_host一致，应该设置为mobile-platform工程所在服务器的内网IP地址(Linux系统路径：tomcat/bin/catalina.sh | windows系统路径:tomcat/bin/catalina.bat)</font>
...
===
####- 需要一个有公网IP和内网IP的机器
####- <font color="red">需要有映射到该服务器公网IP的域名</font>
####- 微信工程名为xxxweixinqyoa（xxx是各个项目具体的微信客户端前缀）
####- JDK版本建议1.6
####- 需要一个单独运行的80端口的tomcat
####- 启动tomcat后，需要在企业号应用中进行回调模式的验证
####PS：需要特殊强调一下回调模式的验证：需要管理员用户登录到微信企业号管理平台，然后点击【应用中心】->选择想要验证的应用->【回调模式】->【回调URL及密钥】修改 -> 将配置文件中的回调URL及密钥填写到对应位置<font color="red">（注：回调URL为 http:// 域名 + /xxxweixinqyoa/weixincore/api）</font> -> 验证成功
####- <font color="red">在企业微信号客户端，管理员输入"createMenu"创建菜单，如果收到"创建菜单成功"则表示创建菜单成功</font>
...
####- <font color="red">需要与提供者确认messagemanagement工程是否为完整工程，验证标准为正确配置后在WEB端的OA正常流程创建人员并且该人员已经被正常加入到微信企业号通讯录中，使用一个已有账号向该人员发送邮件，该人员微信能够收到推送，并且点击能够查看邮件内容。</font>
</br></br>[回到首页](#home)
